...
function showScreen(timerNumber) {
  ...
  document.querySelector('#timer' + timerNumber + ' .timer-title').innerHTML =
    localStorage.getItem(`title${timerNumber}`) || `Select Cutter`;
  ...
  document.querySelector(
    ...
    localStorage.getItem(`subtitle${timerNumber}`) || `Cutter Challenge`;
  document.querySelector(
    '#timer' + timerNumber + ' .timer-subtitle'
  ).style.display = 'block';
}
  ...
  ['timer1', 'timer2', 'timer3'].forEach((id, index) => {
    ...
    if (titleElement) {
      titleElement.innerHTML =
        localStorage.getItem(`title${index + 1}`) || `Select Cutter`;
      titleElement.style.display = 'block';
    }
    if (subtitleElement) {
      subtitleElement.innerHTML =
        localStorage.getItem(`subtitle${index + 1}`) || `Cutter Challenge`;
...
document.addEventListener('DOMContentLoaded', () => {
  showScreen(1);
  // Initialize titles and subtitles from localStorage or default values
  ['1', '2', '3'].forEach((index) => {
    const title = localStorage.getItem(`title${index}`) || `Select Cutter`;
    const subtitle =
      localStorage.getItem(`subtitle${index}`) || `Cutter Challenge`;
    document.querySelector(`#timer${index} .timer-title`).innerHTML = title;
    document.querySelector(`#timer${index} .timer-subtitle`).innerHTML =
      subtitle;
  });
});
